...
if check_argument 'kubelet' '--anonymous-auth=false' >/dev/null 2>&1; then
    pass "$check_2_1_2"
else
    warn "$check_2_1_2"
fi
...
if check_argument 'kubelet' '--authorization-mode=AlwaysAllow' >/dev/null 2>&1; then
    warn "$check_2_1_3"
else
    pass "$check_2_1_3"
fi
...
if check_argument 'kubelet' '--protect-kernel-defaults=true' >/dev/null 2>&1; then
    pass "$check_2_1_7"
else
    warn "$check_2_1_7"
fi
...
if check_argument 'kubelet' '--keep-terminated-pod-volumes=false' >/dev/null 2>&1; then
    pass "$check_2_1_9"
else
    warn "$check_2_1_9"
fi
...
if check_argument 'kubelet' '--hostname-override' >/dev/null 2>&1; then
    warn "$check_2_1_10"
else
    pass "$check_2_1_10"
fi
